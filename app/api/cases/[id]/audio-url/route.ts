...
/**
 * Emite um token de upload assinado (Fase 1R2, item 6) para o path
 * canônico deste caso no bucket `consultations`. Substitui a antiga
 * policy de Storage `consultations_insert_own`/`upsert:true` — agora o
 * navegador não tem nenhuma policy direta; o token assinado, de uso único
 * e escopo restrito a este path, é a autorização.
 */
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (caseError) {
    console.error('audio-url: falha ao carregar caso', caseError.message);
    return NextResponse.json(
  ...
  if (signError || !signed) {
    console.error('audio-url: falha ao criar signed upload url', signError?.message);
    return NextResponse.json(
